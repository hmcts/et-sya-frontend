...
const logger = getLogger('ClaimantRespondentSectionCheckController');

export default class ClaimantRespondentSectionCheckController {
    ...
    if (req.body?.employmentAndRespondentCheck === 'Yes') {
      ...
      if (!isValid) {
        ...
        const content = getPageContent(req, this.formContent, [
          ...
          TranslationKeys.CLAIMANT_RESPONDENT_SECTION_CHECK,
        ]);
        return res.render(TranslationKeys.CLAIMANT_RESPONDENT_SECTION_CHECK, { ...content });
      }
    }
  ...
  public get = (req: AppRequest, res: Response): void => {
    ...
    res.render(TranslationKeys.CLAIMANT_RESPONDENT_SECTION_CHECK, { ...content });
  };
}
