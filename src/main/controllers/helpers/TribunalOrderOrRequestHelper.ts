    ...
    notifications.forEach(item => {
      ...
      const hasStored =
        item.value.respondCollection?.some(
          r => r.value.from === Applicant.CLAIMANT && r.value.responseState === ResponseStatus.STORED
        ) || false;
...
const checkAndUpdateRespondCollection = (
  ...
  if (respond.value.from === Applicant.CLAIMANT && respond.value.responseState === ResponseStatus.STORED) {
    order.redirectUrl =
    ...
    notification =>
      notification.value.respondCollection
        ?.filter(
          response =>
            response.value.from === Applicant.CLAIMANT &&
            response.value.responseState !== ResponseStatus.VIEWED &&
            response.value.responseState !== ResponseStatus.STORED
        )
        .map(response => ({
