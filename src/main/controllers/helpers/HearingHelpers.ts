 ...
 * @param notification userCase sendNotificationCollection type item
 * @param hearingModels userCase hearingCollection
 */
export const isHearingClaimantStateViewed = (
  ...
  return (
    ...
    isAnyListedDateInFuture(notification.value, hearingModels)
  );
};

const isAnyListedDateInFuture = (notification: SendNotificationType, hearingModels: HearingModel[]): boolean => {
  const selectedHearingId = notification.sendNotificationSelectHearing?.selectedCode;
  if (!selectedHearingId) {
    return false;
  }

  const hearingModel = hearingModels?.find(model =>
    model.value.hearingDateCollection.some(hearingDate => hearingDate.id === selectedHearingId)
  );
  if (!hearingModel) {
    return false;
  }

  const hearingDateCollectionItem = hearingModel.value.hearingDateCollection.find(
    hearingDate => hearingDate.id === selectedHearingId
  );
  if (!hearingDateCollectionItem) {
    return false;
  }

  const listedDate = hearingDateCollectionItem.value.listedDate;
  const now = new Date();
  return listedDate > now;
};
