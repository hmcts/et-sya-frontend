...
describe('Onboarding (home) page', () => {
  ...
  it('should display firt paragraph', () => {
    ...
    expect(p[4].innerHTML).contains(expectedP1, 'P1 does not exist');
  });
  ...
  it('should display second paragraph', () => {
    const p = htmlRes.getElementsByClassName(pClass);
    expect(p[5].innerHTML).contains(expectedP2, 'P2 does not exist');
  });
  ...
  it('should display start now button', () => {
    ...
    expect(button[4].innerHTML).contains('Start now', 'Could not find the button');
  });
});
