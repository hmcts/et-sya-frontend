...
const updatePreferenceJsonRaw = fs.readFileSync(
  path.resolve(__dirname, '../../../main/resources/locales/en/translation/present-employer.json'),
  'utf-8'
);
const updatePreferenceJson = JSON.parse(updatePreferenceJsonRaw);

const PAGE_URL = '/present-employer';
...
describe('Are you still working for the organisation or person who dismissed you?', () => {
  beforeAll(async () => {
    await request(app)
      .get(PAGE_URL)
      .then(res => {
