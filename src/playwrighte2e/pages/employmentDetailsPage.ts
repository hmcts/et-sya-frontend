...
export class EmploymentDetailsPage extends BasePage {
   ...
   async didYouWorkForOrg(orgOption: string) {
        ...
        if(orgOption === 'Yes') {
            ...
            await this.clickSaveAndContinue();
            await this.webAction.verifyTextIsVisible("Are you still working for the organisation or person you're making your claim against?");
        }
    }

    async areYouStillWorkingForOrg(stillWorkingOption: string) {
    ...
    async stillWorkingForRespondentJourney(noticePeriodContract: string, noticePeriod: string) {
    ...
    async enterRespondantDetailsJourney(workAddress: string, doYouHaveAcas:string) {
        ...
        if (doYouHaveAcas === 'Yes') {
            ...
            await this.clickSaveAndContinue();
            await this.webAction.verifyTextIsVisible('text=Please note: incorrectly claiming an exemption may lead to your claim being rejected. If in doubt, please contact Acas.');
