...
export class ClaimDetailsPage extends BasePage {
    ...
    async claimDet(allClaimDetailsPages = true){
        ...
        if(allClaimDetailsPages) {
            ...
            await this.webAction.verifyTextIsVisible('text=Race (including colour, nationality, and ethnic or national origins)');
            ...
            await this.webAction.verifyTextIsVisible('text=Use this box to describe the events around your dispute, or add to your claim by uploading a document');
            ...
            await this.webAction.verifyTextIsVisible('text=Try to give the dates of the events (and set them out in date order) and the');
            await this.webAction.verifyTextIsVisible('text=names of other people involved. Explain, as best you can, why you consider');
            await this.webAction.verifyTextIsVisible('text=that these events mean that you have been unlawfully discriminated');
            await this.webAction.verifyTextIsVisible('text=against, by reference to the protected characteristics you rely on.');
            ...
            await this.webAction.verifyTextIsVisible('text=Try to give the dates of the events (like dismissal meetings) and the names');
            await this.webAction.verifyTextIsVisible('text=of others involved (such as the person who dismissed you). You should ex');
            await this.webAction.verifyTextIsVisible('text=plain, as best you can, why you consider that your dismissal was unfair.');
            ...
            await this.webAction.verifyTextIsVisible('text=Give the dates of each time you say you "blew the whistle" by making a');
            await this.webAction.verifyTextIsVisible('text=protected disclosure. For each disclosure summarise the information you');
            await this.webAction.verifyTextIsVisible('text=disclosed, whether it was done verbally or in writing, the person to whom');
            await this.webAction.verifyTextIsVisible('text=you made the disclosure, what kind of wrongdoing you were disclosing,');
            await this.webAction.verifyTextIsVisible('text=and why the disclosure was in the public interest.');
            await this.webAction.verifyTextIsVisible('text=Explain how you were treated detrimentally (which can include being dismissed) because of the disclosure(s), giving dates and summarising the incidents.');
            ...
            await this.webAction.verifyTextIsVisible('text=The tribunal can only hear cases where legislation gives it power to do so.');
            await this.webAction.verifyTextIsVisible('text=Explain briefly, if you can, what legislation you say covers this claim.');
            await this.webAction.verifyTextIsVisible('text=Even if you cannot, the most important thing is to explain what was done');
            await this.webAction.verifyTextIsVisible('text=and, so far as you can, why you say it was unlawful. Give dates and brief');
            await this.webAction.verifyTextIsVisible('text=details of each incident, including the name of the person responsible.');
            ...
            await this.webAction.verifyTextIsVisible('text=If claiming unfair dismissal, to get your old job back and compensation');
            await this.webAction.verifyTextIsVisible('text=If claiming unfair dismissal, to get another job with the same employer or associated employer and compensation (re-engagement)');

            // await this.webAction.clickElementByText('text=Compensation - what can a tribunal award?');
            await this.page.locator('summary span').nth(0).click();
            await this.webAction.verifyTextIsVisible('text=If a tribunal decides you’ve been unfairly dismissed, you’ll normally receive');
            ...
            await this.webAction.verifyTextIsVisible('text=a compensatory award - compensation for the actual money you lost');
            await this.webAction.verifyTextIsVisible('text=due to your dismissal, or which you expect to lose in future because of');
            await this.webAction.verifyTextIsVisible('text=your dismissal');
            await this.webAction.verifyTextIsVisible('text=Compensation can be reduced for various reasons and a tribunal will tell you');
            await this.webAction.verifyTextIsVisible('text=more about this if it applies in your specific circumstances.');
            await this.webAction.verifyTextIsVisible('text=In disciplinary or grievance cases, you may be entitled to an increased award');
            await this.webAction.verifyTextIsVisible('text=if an employer fails to follow the guidance set out in the Acas Code of');
            await this.webAction.verifyTextIsVisible('text=Practice on disciplinary and grievance procedures (opens in new tab).');
            await this.webAction.verifyTextIsVisible("text=Similarly, awards can be reduced if a tribunal decides you've failed to follow");
            await this.webAction.verifyTextIsVisible('text=the guidance set out in the Acas Code of Practice on disciplinary and grievance procedures (opens in new tab).');
            await this.webAction.verifyTextIsVisible('text=In certain claims, such as those involving discrimination, a tribunal may com');
            await this.webAction.verifyTextIsVisible("text=pensate you for 'injury to feelings' (individual hurt and distress you may have");
            ...
            await this.page.locator('summary span').nth(1).click();
            await this.webAction.verifyTextIsVisible('text=If your employer is found to have discriminated against you, a tribunal can');
            await this.webAction.verifyTextIsVisible('text=make a recommendation that the respondent take specific steps to reduce');
            ...
            await this.webAction.verifyTextIsVisible('text=Compensation - what can a tribunal award?');
            await this.webAction.verifyTextIsVisible('text=Try to set out all compensation you’re claiming for, and provide a total if possible');
            ...
            await this.webAction.verifyTextIsVisible('text=What is a tribunal recommendation?');
            await this.webAction.verifyTextIsVisible('text=Tell us what action you’d like a tribunal to recommend your respondents take to');
            ...
            await this.webAction.verifyTextIsVisible('text=You’ve selected a whistleblowing claim, so you can request that we forward a');
            await this.webAction.verifyTextIsVisible('text=copy of your claim to a relevant regulator (also known as ‘prescribed person’) or');
            await this.webAction.verifyTextIsVisible('text=body.');
            await this.webAction.verifyTextIsVisible('text=We will notify the respondent if you choose for us to forward your claim but this');
            await this.webAction.verifyTextIsVisible('text=will not affect how we process your claim');
            await this.webAction.verifyTextIsVisible('text=Not all whistleblowing claims will have an appropriate regulator or body');
            await this.webAction.verifyTextIsVisible('text=Find the relevant regulator or body (opens in a new window) for your type of');
            await this.webAction.verifyTextIsVisible('text=claim from this list');
            await this.webAction.verifyTextIsVisible('text=Do you want us to forward your whistleblowing claim to a relevant');
            ...
            await this.webAction.verifyTextIsVisible('text=Tell us if there are any existing cases this claim could be linked to.');
            ...
            await this.webAction.verifyTextIsVisible('text=a case or cases brought by other people against the same employer with the same or similar circumstances');
            await this.webAction.verifyTextIsVisible('text=This will help the tribunal consider whether the cases should be linked in any way.');
            await this.webAction.verifyTextIsVisible('text=Are there are any existing cases which may be linked to this new claim? (Optional)');
